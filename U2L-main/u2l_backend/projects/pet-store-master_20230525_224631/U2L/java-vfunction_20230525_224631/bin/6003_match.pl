#!/usr/bin/perl
#
# $Header: /data/cvsrepo/assess-tools/bin/6003_match.pl,v 1.2 2013/06/24 05:36:29 miya Exp $
# $Name: rev_1_10 $
#
# ツール名 : 6003_match.pl
# 概要     : キーワード抽出スクリプト
# 実行方法 : 6003_match.pl <抽出キーワード>ファイル名
# 入力	   : <抽出キーワード> ファイルで指定する
...
while(<STDIN>) {
	...
	for(local $i=0; $i < $#patterns; $i++) {
		...
		if ($substr =~ /$pat/) {	# 1つでも見つかったらその行を出力
			print $_; 
			last;
		}
	}
